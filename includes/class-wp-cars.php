...
class Wp_Cars {
	...
	private function define_public_hooks() {
		...
		$this->loader->add_action( 'wp_ajax_show_next_post', $plugin_public, 'show_next_post' );
		$this->loader->add_action( 'wp_ajax_nopriv_show_next_post', $plugin_public, 'show_next_post' );
	...
	/**
	 * Retrive Plugin options from database.
	 *
	 * @since     1.0.0
	 * @return    array
	 */
	public static function get_options() {
		return (array)get_option('wp-cars-plugin');
	}

	/**
	 * Update Plugin options in database.
	 *
	 * @since     1.0.0
	 * @param     array $plugin_options
	 * @return    boolean
	 */
	public static function update_options($plugin_options = array()) {
		return update_option('wp-cars-plugin', $plugin_options);
	}

	/**
	 * Description: Filter input data from $_POST, $_GET, etc.
	 *
	 * @param MIXED $data
	 * @param STRING $type
	 * @param STRING $case
		...
		switch($type)
		{
			...
			default:
				if ( 'lower' == $case ) {
					...
					return strtoupper(nl2br(htmlspecialchars(stripslashes(trim(strip_tags($data))))));
				}
				else{
